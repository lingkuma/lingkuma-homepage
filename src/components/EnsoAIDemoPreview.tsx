...
// Mock data
const repositories = [
  { name: 'awesome-app', path: '~/projects/awesome-app', expanded: true },
  { name: 'design-system', path: '~/projects/design-system', expanded: false },
];
...
export function EnsoAIDemoPreview() {
  ...
  // Tree sidebar - expanded repos state
  const [expandedRepos, setExpandedRepos] = useState<Set<string>>(new Set(['awesome-app']));

  const toggleRepoExpanded = useCallback((repoName: string) => {
    ...
    >
      ...
      <div className="rounded-xl overflow-hidden shadow-2xl border border-ayu-line/50 bg-ayu-panel">
        ...
        <div className="flex h-[680px] bg-ayu-bg">
          {/* Tree Sidebar - Combined Repository & Worktree List */}
          <motion.div
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.5, delay: 0.4 }}
            className="w-72 border-r border-ayu-line bg-ayu-panel flex flex-col"
          >
            {/* Search */}
            <div className="p-3 border-b border-ayu-line">
              <div className="flex items-center gap-2 px-3 py-1.5 bg-ayu-bg rounded-md border border-ayu-line/50">
                ...
                <span className="text-xs text-ayu-fg/40">{t('demo.search')}</span>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto py-2">
              {repositories.map((repo, repoIndex) => {
                ...
                const repoWorktrees = worktreesData[repo.name] || [];
                ...
                return (
                  <motion.div
                    ...
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ duration: 0.3, delay: 0.5 + repoIndex * 0.1 }}
                  >
                    {/* Repository Row */}
                    <div
                      className={`mx-2 px-2 py-1.5 rounded-lg cursor-pointer transition-all flex items-center gap-1 ${
                        isSelected && !isExpanded
                          ? 'bg-ayu-accent/10'
                          : 'hover:bg-ayu-line/30'
                      }`}
                    >
                      ...
                      <button
                        ...
                        className="w-5 h-5 flex items-center justify-center rounded hover:bg-ayu-line/50"
                      >
                        {isExpanded ? (
                          <ChevronDown className="w-3.5 h-3.5 text-ayu-fg/40" />
                        ) : (
                          <ChevronRight className="w-3.5 h-3.5 text-ayu-fg/40" />
                        )}
                      </button>
                      {/* Repository Name */}
                      <div
                        onClick={() => {
                          ...
                          // Select first worktree (main)
                          const firstWorktree = repoWorktrees[0];
                          if (firstWorktree) {
                            setActiveWorktree(firstWorktree.name);
                          }
                        }}
                        className="flex-1 flex items-center gap-2 min-w-0"
                      >
                        {isExpanded ? (
                          <FolderOpen className={`w-4 h-4 shrink-0 ${isSelected ? 'text-ayu-accent' : 'text-ayu-fg/60'}`} />
                        ) : (
                          <Folder className={`w-4 h-4 shrink-0 ${isSelected ? 'text-ayu-accent' : 'text-ayu-fg/60'}`} />
                        )}
                        <span className={`text-sm font-medium truncate ${isSelected ? 'text-ayu-accent' : 'text-ayu-fg'}`}>
                          {repo.name}
                        </span>
                      </div>
                    </div>
                    ...
                    {isExpanded && (
                      <div className="ml-4 mt-0.5 space-y-0.5">
                        {repoWorktrees.map((wt, wtIndex) => {
                          const isActiveWt = selectedRepo === repo.name && activeWorktree === wt.name;
                          return (
                            <motion.div
                              key={wt.name}
                              initial={{ opacity: 0, x: -10 }}
                              animate={{ opacity: 1, x: 0 }}
                              transition={{ duration: 0.15, delay: wtIndex * 0.02 }}
                              onClick={() => {
                                setSelectedRepo(repo.name);
                                setActiveWorktree(wt.name);
                              }}
                              className={`mx-2 px-2 py-1.5 rounded-lg cursor-pointer transition-all ${
                                isActiveWt
                                  ? 'bg-ayu-accent/10 border border-ayu-accent/30'
                                  : 'hover:bg-ayu-line/30 border border-transparent'
                              }`}
                            >
                              <div className="flex items-center gap-2">
                                <GitBranch className={`w-3.5 h-3.5 shrink-0 ${isActiveWt ? 'text-ayu-accent' : 'text-ayu-fg/50'}`} />
                                <span className={`text-sm truncate flex-1 ${isActiveWt ? 'text-ayu-accent font-medium' : 'text-ayu-fg'}`}>
                                  {wt.name}
                                </span>
                                {wt.badge && (
                                  <span className="px-1.5 py-0.5 text-[9px] font-semibold bg-ayu-green/20 text-ayu-green rounded shrink-0">
                                    {wt.badge}
                                  </span>
                                )}
                                {wt.hasChanges && (
                                  <CircleDot className="w-3 h-3 text-ayu-green shrink-0" />
                                )}
                                {wt.count && (
                                  <span className="w-4 h-4 flex items-center justify-center text-[10px] bg-ayu-fg/10 text-ayu-fg/70 rounded-full shrink-0">
                                    {wt.count}
                                  </span>
                                )}
                              </div>
